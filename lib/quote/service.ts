...
export async function buildPreviewQuotePdf(formData: FormData) {
  ...
  const quote = calculateQuote(parsed.input);
  if (!quote) {
    return { ok: false as const, message: "Angebot konnte nicht berechnet werden." };
  }
  ...
  return {
    ok: true as const,
    buffer: pdfBuffer,
    filename: `Test-${quote.quoteNumber}.pdf`,
    quote,
  };
}

export async function submitQuoteRequest(formData: FormData): Promise<QuoteFormState> {
  ...
  try {
    ...
    return {
      ...
      message: emailResult.sent
        ? "Vielen Dank! Ihr unverbindliches Angebot wurde erstellt und per E-Mail an Sie gesendet. Tippen Sie unten auf „PDF öffnen“."
        : "Vielen Dank! Ihr unverbindliches Angebot wurde erstellt. Tippen Sie unten auf „PDF öffnen“ – der E-Mail-Versand wird nach SMTP-Konfiguration aktiviert.",
...
export function parseFilenameFromDisposition(header: string | null) {
